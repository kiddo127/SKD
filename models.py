...
def vit_tiny(pretrained=True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/deit/deit_tiny_patch16_224-a1311bcf.pth", # 72.14
            map_location="cpu",
            check_hash=True
        )
...
def vit_small(pretrained=True, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        url = "https://dl.fbaipublicfiles.com/deit/deit_small_patch16_224-cd65a155.pth" # 79.83
        checkpoint = torch.hub.load_state_dict_from_url(
            url=url,
            map_location="cpu",
            check_hash=True,
...
def vit_base(pretrained=True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/deit/deit_base_patch16_224-b5f2ef4d.pth", # 81.80
...
def create_model(model, pretrained, num_classes):
    if 'vit_base' in model:
        ...
        model = deit_base_patch16_224(pretrained=pretrained, num_classes=num_classes)
    elif 'deit_small_patch16_224' in model:
        model = deit_small_patch16_224(pretrained=pretrained, num_classes=num_classes)
    else:
